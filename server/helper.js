...
const setPassword = function (line, socket, rooms, roomsSeen, playerRoom) {
  for (const room of rooms) {
    if (
      line.startsWith(
        `'User ${room} with bot AI "my-bot" spawned in ${room}'`
      )
    ) {
      roomsSeen[room] = true;
      ...
      if (process.env.STEAM_ID  && room === playerRoom) {
        ...
        socket.write(
          `storage.db.users.update({username: '${room}'}, {$set: {steam: {id: '${process.env.STEAM_ID}'}}})\r\n`
        );
      }
      return true;
    }
...
async function initServer() {
  ...
  await new Promise((resolve) => {
    ncp(
      path.resolve(__dirname, "../node_modules/@screeps/launcher/init_dist"),
      dir,
      (e) => {
        resolve();
      }
...
const logConsole = function (room) {
  return (event) => {
    if (event.channel !== "console") {
      console.log(
        room,
        `logConsole channel not console: ${JSON.stringify(event)}`
      );
    }
    if (event.type !== "user") {
      console.log(
        room,
        `logConsole channel not user: ${JSON.stringify(event)}`
      );
    }
    if (!event.data) {
    ...
    if (event.data.messages) {
      if (event.data.messages.results.length > 0) {
        console.log(
          room,
          `logConsole event.data.messages.results: ${JSON.stringify(
            event.data.messages.results
          )}`
        );
      }
      ...
      if (event.data.messages.log.length > 0) {
        for (
          let logIndex = 0;
          logIndex < event.data.messages.log.length;
          logIndex++
        ) {
...
const spawnBots = async function (line, socket, rooms, players, tickDuration) {
  if (line.startsWith(`Screeps server v`)) {
    ...
    for (const room of ["W0N0", "W10N0", "W10N10", "W0N10"]) {
      console.log(
        `> storage.db['rooms.objects'].insert({ type: 'terminal', room: '${room}', x: 0, y:0 })`
      );
      socket.write(
        `storage.db['rooms.objects'].insert({ type: 'terminal', room: '${room}', x: 0, y:0 })\r\n`
      );
